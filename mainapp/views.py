...
from mainapp.models import Cart, Category, Customer, Product, CartProduct
...
class CategoryDetailView(CartMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not query or not self.request.GET:
        ...
        products = Product.objects.filter(id_in=[pf['product_id'] for pf in pf])
        context['category_products'] = products
        return context
...
class DeleteFromCartView(CartMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        cart_product.delete()
        recalc_cart(self.cart)
        messages.add_message(request, messages.INFO, 'Товар успешно дал по сьебам')
        return HttpResponseRedirect('/cart/')
